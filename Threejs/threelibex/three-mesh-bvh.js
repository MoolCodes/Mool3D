...
function getRootIndexRanges(geo) {
  ...
  const sortedBoundaries = Array.from(rangeBoundaries.values()).sort(
    (a, b) => a - b
  );
  ...
  while (true) {
    ...
    if (left < right) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        triangleBounds[left * 6 + i * 2 + 0] =
          triangleBounds[right * 6 + i * 2 + 0];
        triangleBounds[right * 6 + i * 2 + 0] = t1;

        let t2 = triangleBounds[left * 6 + i * 2 + 1];
        triangleBounds[left * 6 + i * 2 + 1] =
          triangleBounds[right * 6 + i * 2 + 1];
    ...
    for (let a = 0; a < 3; a++) {
      ...
      if (count < BIN_COUNT / 4) {
        ...
        for (let bi = 0; bi < splitCount; bi++) {
          ...
          while (
            bi + 1 < splitCount &&
            truncatedBins[bi + 1].candidate === bin.candidate
          ) {
        ...
        for (let i = BIN_COUNT - 2; i >= 0; i--) {
          ...
          unionBounds(
            bin.bounds,
            nextBin.rightCacheBounds,
            bin.rightCacheBounds
          );
        }
...
function buildTree(geo, options) {
  ...
  function splitNode(
    ...
    if (!reachedMaxDepth && depth >= maxDepth) {
      ...
      if (verbose) {
        console.warn(
          `MeshBVH: Max depth of ${maxDepth} reached when generating BVH. Consider increasing maxDepth.`
        );
        console.warn(geo);
      }
    }
...
const closestPointsSegmentToSegment = (function() {
  ...
  return function closestPointsSegmentToSegment(l1, l2, target1, target2) {
    ...
    if (d >= 0 && d <= 1 && d2 >= 0 && d2 <= 1) {
      ...
      if (
        closestPoint.distanceToSquared(p2) <= closestPoint2.distanceToSquared(p)
      ) {
...
function raycastFirst(nodeIndex32, geometry, side, ray) {
  ...
  if (isLeaf) {
    ...
    const c1Result = c1Intersection
      ? raycastFirst(c1, geometry, side, ray)
      : null;
    ...
    const c2Result = c2Intersection
      ? raycastFirst(c2, geometry, side, ray)
      : null;
  ...
  function shapecastTraverse(
    ...
    if (isLeaf) {
      ...
      return intersectsRangeFunc(
        ...
        false,
        depth,
        nodeIndexByteOffset + nodeIndex32,
        _box1
      );
    } else {
      ...
      if (c2Intersection === CONTAINED) {
        ...
        c2StopTraversal = intersectsRangeFunc(
          offset,
          count,
          true,
          depth + 1,
...
const intersectsGeometry = (function() {
  ...
  return function intersectsGeometry(
    nodeIndex32,
    geometry,
    otherGeometry,
    geometryToBvh,
    ...
    if (cachedObb === null) {
      ...
      obb.set(
        otherGeometry.boundingBox.min,
        otherGeometry.boundingBox.max,
        geometryToBvh
      );
      cachedObb = obb;
    }
...
const trianglePool = /* @__PURE__ */ new PrimitivePool(
  () => new SeparatingAxisTriangle()
);
...
class MeshBVH {
      ...
      if (isLeaf) {
        ...
        if (didChange) {
          for (let i = 0; i < 3; i++) {
            ...
            float32Array[node32Index + i] =
              minLeftValue < minRightValue ? minLeftValue : minRightValue;
            float32Array[node32Index + i + 3] =
              maxLeftValue > maxRightValue ? maxLeftValue : maxRightValue;
          }
        }
  ...
  raycast(ray, materialOrSide = FrontSide) {
    ...
    for (let i = 0, l = roots.length; i < l; i++) {
      const materialSide = isArrayMaterial
        ? materialOrSide[groups[i].materialIndex].side
        : side;
  ...
  shapecast(callbacks, _intersectsTriangleFunc, _orderNodesFunc) {
    ...
    if (intersectsRange && intersectsTriangle) {
      ...
      intersectsRange = (offset, count, contained, depth, nodeIndex) => {
        if (
          !originalIntersectsRange(offset, count, contained, depth, nodeIndex)
        ) {
          return iterateOverTriangles(
            offset,
            count,
            geometry,
            intersectsTriangle,
  ...
  bvhcast(otherBvh, matrixToLocal, callbacks) {
    ...
    if (intersectsTriangles) {
      function iterateOverDoubleTriangles(
        offset1,
        count1,
        offset2,
        count2,
      ...
      intersectsRange: (offset, count) => {
        if (otherGeometry.boundsTree) {
          ...
          return otherGeometry.boundsTree.shapecast({
            boundsTraverseOrder: box => {
              return obb2.distanceToBox(
                box,
                Math.min(closestDistance, maxThreshold)
              );
            },
            ...
            intersectsRange: (otherOffset, otherCount) => {
              for (
                let i2 = otherOffset * 3, l2 = (otherOffset + otherCount) * 3;
                i2 < l2;
                i2 += 3
              ) {
                ...
                for (
                  let i = offset * 3, l = (offset + count) * 3;
                  i < l;
                  i += 3
                ) {
                  ...
                  const dist = triangle.distanceToTriangle(
                    triangle2,
                    tempTarget1,
                    tempTarget2
                  );
...
MeshBVH.prototype.raycastFirst = function(...args) {
  if (args[0].isMesh) {
    ...
    return convertRaycastIntersect(
      originalRaycastFirst.call(this, ray, mesh.material),
      mesh,
      raycaster
    );
  } else {
...
MeshBVH.prototype.refit = function(...args) {
  ...
  if (
    terminationIndices &&
    (terminationIndices instanceof Set || Array.isArray(terminationIndices))
  ) {
    console.warn(
...
[
  ...
  MeshBVH.prototype[name] = function(...args) {
    if (args[0] === null || args[0].isMesh) {
      args.shift();
      console.warn(
        `MeshBVH: The function signature for "${name}" has changed and no longer takes Mesh. See docs for new signature.`
      );
    }
  ...
  update() {
    ...
    for (let i = 0; i < totalRoots; i++) {
      if (i >= this._roots.length) {
        const root = new MeshBVHRootVisualizer(
          this.mesh,
          this.edgeMaterial,
          this.depth,
          i
  ...
  bvh.traverse((depth, isLeaf, boundingData, offset, count) => {
    ...
    if (isLeaf) {
      ...
      for (let i = offset * 3, l = (offset + count) * 3; i < l; i += 3) {
        ...
        console.assert(
          isContained,
          'Leaf bounds does not fully contain triangle.'
        );
    ...
    if (parent) {
      ...
      console.assert(
        isContained,
        'Parent bounds does not fully contain child.'
      );
      passes = passes && isContained;
    }
  });
...
class VertexAttributeTexture extends DataTexture {
  ...
  updateFrom(attr) {
    ...
    switch (targetType) {
      ...
      case IntType:
        ...
        normalizeValue = normalized
          ? Math.pow(2, originalBufferCons.BYTES_PER_ELEMENT * 8 - 1)
          : 1.0;
    ...
    if (
      finalStride === 3 &&
      (format === RGBAFormat || format === RGBAIntegerFormat)
    ) {
      finalStride = 4;
    }
...
function bvhToTextures(bvh, boundsTexture, contentsTexture) {
  ...
  for (let i = 0; i < nodeCount; i++) {
    ...
    if (IS_LEAF(nodeIndex16, uint16Array)) {
      ...
      const rightIndex =
        (4 * RIGHT_NODE(nodeIndex32, uint32Array)) / BYTES_PER_NODE;
      const splitAxis = SPLIT_AXIS(nodeIndex32, uint32Array);
